...
function GoInput({ id, children, value, onChange, required }) {
  ...
  return (
    <label
      htmlFor={id}
      className={`input-group ${content.length > 0 ? "has-content" : ""}`}
    >
      <input
        required={required}
        type="text"
...
function LocationEntry({ id, children, onDecision, forceValue, bias }) {
  ...
  return (
    <div
      className={`relative ${potential?.suggestions?.length > 0 ? "has-results" : ""}`}
    >
      <GoInput
        id={id}
        onChange={(value) => {
      ...
      {potential?.suggestions?.length > 0 && (
        <div
          className={
            "bg-black/75 absolute flex flex-col gap-2 p-2 rounded-b-lg max-h-48 overflow-x-auto w-full z-10"
          }
        >
          {potential.suggestions.map((pobj) => {
            ...
            return (
              <button
                ...
                className={
                  "outline-grey rounded-md p-2 outline-1 -outline-offset-1 text-sm flex flex-col gap-1 text-left"
                }
              >
    ...
    <>
      {routes?.length > 0 ? (
        <>
                    ...
                    <div className={"flex gap-1"}>
                      <span>
                        {r.routes[0].distance < 1000 ? (
                          <>
                            <span className={"font-bold"}>
                              {Math.floor(r.routes[0].distance)}
                            </span>
                            m
                          </>
                        ) : (
                          <>
                            <span className={"font-bold"}>
                              {(r.routes[0].distance / 1000).toFixed(1)}
                            </span>
                            km
                          </>
                        )}
          ...
          <button
            disabled={chosenRoute === undefined}
            onClick={() => {
...
function DoRoute({ route }) {
  const { updateMapProps, setLocation, fitBounds, setRoutes, getMapRef } =
    useMap();
  ...
  useEffect(() => {
    ...
    const watchId = navigator.geolocation.watchPosition(
      (geo) => {
        updateMapProps({
          userPosition: [geo.coords.longitude, geo.coords.latitude],
        });
        if (!position)
          setLocation({
  ...
  useEffect(() => {
    ...
    el.setAttribute(
      "style",
      "position: absolute; top: 50%; left: 50%; transform: translate(-50%, -50%); background: #fff; border-radius: 100%; width: 4px; height: 4px; z-index: 100000; pointer-events: none;",
    );
  ...
  useEffect(() => {
    navigator.geolocation.getCurrentPosition(
      (geo) => {
        ...
        setUserPosition([geo.coords.longitude, geo.coords.latitude]);
        setLocation({
          longitude: geo.coords.longitude,
          latitude: geo.coords.latitude,
          zoom: 12,
      ...
      >
        {!fromTo ? (
          <ChooseDestinations
            ...
            onDecision={async (from, to) => {
              setRouteMessage("");
              setFromTo([from, to]);
              const fromLocation = from.location;
              const toLocation = to.location;
              if (
                !isWithinGreaterLondon(fromLocation) ||
                !isWithinGreaterLondon(toLocation)
              ) {
                setRoutesState([]);
                setRouteMessage(
                  "Route planning currently only works within Greater London.",
                );
                return;
              }
              ...
              if (!Array.isArray(calculated)) {
                setRoutesState([]);
                setRouteMessage(
                  "We couldn't calculate that route right now. Route planning currently only works within Greater London.",
                );
                return;
              }
              if (calculated.length === 0) {
                setRouteMessage(
                  "No route is available for that journey. Route planning currently only works within Greater London.",
                );
              }
          ...
          <>
            ...
            {routeMessage ? (
              <div
                className={
                  "rounded-md bg-amber-500/15 px-3 py-2 text-sm text-amber-100 outline outline-1 -outline-offset-1 outline-amber-400/40"
                }
              >
                {routeMessage}
              </div>
