...
describe("Learn page", async () => {
  ...
  it("shows a loading message", () => {
    ...
    expect(
      page.getByText(/loading educational resources/i),
    ).toBeInTheDocument();
  });
  ...
  it("shows error when API fails", async () => {
    vi.mocked(getEducationalResources).mockRejectedValueOnce(
  ...
  it("opens the crime type filter and selects a crime type", async () => {
    ...
    const allResourcesButton = document.querySelector(
    ...
    const burglaryButton = document.querySelector(
      '[data-testid="crime-option-burglary"]',
    );
    ...
    const burglaryResource = Array.from(
      document.querySelectorAll("div.space-y-6 h2"),
    ).find((h2) => h2.textContent === "Burglary Tips");
    expect(burglaryResource).toBeDefined();

    const robberyResource = Array.from(
      document.querySelectorAll("div.space-y-6 h2"),
    ).find((h2) => h2.textContent === "Robbery Tips");
    expect(robberyResource).toBeDefined();
  });
});
